 ...
 * Overwrites MCRMODSLinksEventHandler from mycore-base.
 *
 * @author Thomas Scheffler (yagee)
 */
public class UBOMODSLinksEventHandler extends MCREventHandlerBase {
    ...
    protected void handleObjectCreated(final MCREvent evt, final MCRObject obj) {
        ...
        if (!joinedNodes.isEmpty()) {
            for (Element linkingNode : joinedNodes) {
                ...
                UBOMODSRelationshipType relType = UBOMODSRelationshipType.fromValue(relationshipTypeRaw);
                //MCR-1328 (no reference links for 'host')
                if (relType != UBOMODSRelationshipType.HOST) {
