    ...
    <div class="form-row mb-1">
      ...
      <div class="col-5">
        <select id="filterType" v-model="model.filterType" class="form-control">
          ...
          <option value="name">{{ i18n["dedup.nav.DedupListView.filter.name"] }}</option>
          <option value="nameidentifier">{{ i18n["dedup.nav.DedupListView.filter.identifier"] }}
          </option>
        </select>
      </div>
    </div>
      ...
      <div class="col-12">
        <table class="table table-hover">
          ...
          <tr v-for="entry in model.dedupList"
            ...
            <td>
              <span
                v-if="entry.deduplicationType == 'name'">{{ i18n["dedup.nav.DedupListView.filter.name"] }}</span>
              <span
                v-else-if="entry.deduplicationType == 'nameidentifier'">{{ i18n["dedup.nav.DedupListView.filter.identifier"] }}</span>
            </td>
            <td>
              <PersonDisplay :mcr-id="entry.mcrId1"/>
            </td>
            <td>
              <PersonDisplay :mcr-id="entry.mcrId2"/>
            </td>
          </tr>
          </tbody>
        </table>
...
const model = reactive({
  ...
  filterType: "all" as "name" | "identifier" | "all",
...
const i18n = reactive({
  ...
  "dedup.nav.DedupListView.filter.name": "name",
...
const resolveDedupList = async () => {
  ...
  const request = await fetch(getMCRApplicationBaseURL() + "rsc/dedup/list/duplicates-person" + (realParams.length > 0 ? "?" + realParams : ""));
