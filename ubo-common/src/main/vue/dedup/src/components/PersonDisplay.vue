...
import {getMCRApplicationBaseURL, getMODSPersonName, getShortID} from "@/api/Utils.ts";
...
const modsPersonName = computed(() => {
  if (model.mods === null) {
    return null;
  } else {
    return getMODSPersonName(model.mods);
  }
});
